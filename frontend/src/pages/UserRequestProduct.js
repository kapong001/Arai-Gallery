...
const RequestProduct = () => {
  ...
  const [owner_by, setOwner] = useState("");
  ...
  const handleCreate = (e) => {
    ...
    try {
      axios
        .post(`http://localhost:8080/api/product/product-request`, productData)
        .then((response) => {
              ...
              >
                {categories?.map((c) => (
                  <Option key={c._id} value={c._id}>
                    {c.name ? c.name : "Undefined category"}
                  </Option>
                ))}
              </Select>
              <div className="mb-3">
